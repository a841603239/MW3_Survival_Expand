...
_id_3DE5()
{
    ...
    if ( isdefined( self._id_3D5D ) && issubstr( self._id_3D5D._id_160B, "riotshield" ) )
    {
        ...
        self setengagementmaxdist( 512, 768 );
        self.goalradius = 448;
        thread _id_3DE6(); // 配置防暴盾牌伤害抗性
        thread _id_00D5( 300 ); // 配置GIGN最小受击硬直伤害阈值
        thread GIGN_turn_hold_position(); // 60秒后变更行为策略：转为固守
    }

    if ( self.aitype == "friendly_support_delta" )
    {
        // 配置delta小队作战参数
        self setengagementmindist( 300, 200 );
        self setengagementmaxdist( 512, 768 );
        self.goalradius = 512;
        thread _id_00D5( 500 ); // 配置delta最小受击硬直伤害阈值

    thread friendly_damage_scale(); //自定义伤害抗性
        
    }

    // 通用
    self.ignoreme = 0;          // 不再被友军火力忽略
    self.fixednode = 0;         // 不固定在某个节点
    self.dropweapon = 0;        // 死亡时不掉落武器
    self._id_3DD9 = 1;          // 标记为“精英单位”
    self.drawoncompass = 1;     // 在罗盘（小地图）上显示
    maps\_utility::_id_0D72( 1 ); //落地后，恢复盟友小队的物理碰撞
    self pushplayer( 0 );       // 不推开玩家
    self._id_1A2D = 30;         // (未知用途的属性)

    thread _id_3DE8(); // 小队阵亡后的相关逻辑
    thread _id_3DEC( 4.0, self.goalradius, "stop_following_player" ); // 配置行为策略：跟随玩家
}

GIGN_turn_hold_position()
{
    // 此函数负责GIGN小队在生成60秒后转为原地固守模式
    self endon("death"); // 如果单位死亡，则终止此线程

    // 等待60秒
    wait 60;

    // 发出通知，用于中断_id_3DEC函数的"跟随玩家"行为策略
    self notify("stop_following_player");

    // 将目标半径设为很小的值，并以当前位置为目标，实现原地固守
    self.goalradius = 64;
    self setgoalpos(self.origin);
    iprintlnbold( "[LOG] GIGN squad strategy changed to ^4 HOLD POSITION." );
}
...
// 随着波次增加，敌人血量呈线性增长，参照survival_waves.csv表格的H列。
// 原设定：每波增量 = 基础生命值 × 0.1。
// 模组设定为每波增量 = 基础生命值 × 0.05。在40波时生命值将达到基础的300%，后续不再增长。
_id_3DFA( var_0 )
{
    if ( isdefined( level._id_3D46 ) )
        var_1 = min( 1.0 + level._id_3D46 * 0.05, 3.0 );
    else
        var_1 = 1.0;
...
// 随着波次增加，敌人移动速度呈线性增长，参照survival_waves.csv表格的L列。
// 原设定：每波增量 = 基础移动速度 × 0.02。
// 模组设定为每波增量 = 基础移动速度 × 0.02。在25波时移动速度将达到基础的150%，后续不再增长。
_id_3DFB( var_0 )
{
    if ( isdefined( level._id_3D46 ) )
        var_1 = 1.0 + level._id_3D46 * 0.02;
    else
        var_1 = 1.0;
...
// 随着波次增加，敌人命中精度呈线性增长，参照survival_waves.csv表格的I列。
// 原设定：每波增量 = 基础命中精度 × 0.2。
// 模组设定为每波增量 = 基础命中精度 × 0.01。在20波时命中精度将达到基础的120%，后续不再增长。
_id_3DFC( var_0 )
{
    if ( isdefined( level._id_3D46 ) )
        var_1 = min( 1.0 + level._id_3D46 * 0.01, 1.2 );
    else
        var_1 = 1.0;
...
//
// ─────────────────────────────────────────────
// 友军减伤线程（不再修改 maxhealth）
// ─────────────────────────────────────────────
// 调用方法：spawn/初始化之后执行 thread friendly_damage_scale();
//
friendly_damage_scale()
{
    self endon( "death" );

    // 记录出生时的最大生命值，只做一次
    if ( !isdefined( self.baseMaxHP ) )
        self.baseMaxHP = self.maxhealth;

    for (;;)
    {
        // 等待引擎抛出的 damage 事件
        self waittill( "damage",
                       dmg, attacker, dir, point, mod, model, tag, weapon );

        // 计算自定义伤害系数
        factor  = get_custom_factor( mod, attacker, weapon );
        scaled  = int( dmg * factor );     // 真正应该受到的伤害
        refund  = dmg - scaled;            // 需要退回的量（正值）

        // 若 scaled 足以致死，交给引擎原生流程即可
        if ( scaled >= self.health )
        {
            self.health = 0;               // 触发死亡
            continue;
        }
        ...
        // 引擎会同步 maxhealth → health，此时立即改回
        self.maxhealth = self.baseMaxHP;
    }
}
...
// 计算自定义伤害系数
// ─────────────────────────────────────────────
get_custom_factor( mod, attacker, weapon )
{
    // 示例规则，可根据需要自行调整 ↓↓↓
    if ( isdefined( attacker ) && isai( attacker ) )
        return 0.50;                                   // 敌方 AI 子弹
