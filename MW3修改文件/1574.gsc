...
_id_3EBB()
{
    ...
    // 初始化盟友小队计数为0
    level.alliedSquadCount = 0;
	
    // 初始化盟友小队生命值 HUD 监控线程
    thread mod_friendly_health_hud::monitor_friendly_health();
    ...
    precacheshader( "small_icon_survival_juggernaut" );
    precacheshader( "small_icon_survival_gign" );
    precacheshader( "small_icon_survival_chopper" );
...
_id_3EBE()
{
    ...
    foreach ( var_4 in level.players )
    {
        ...
        // 激活laststand状态监控函数
        var_4 thread _id_0615::monitorShellshockProxy();
    }
}
...
_id_3EDB()
{
    ...
    for (;;)
    {
        ...
        if ( isdefined( var_0.sentrytype ) )
        {
            if ( var_0.sentrytype == "sentry_minigun" )
            {
                var_0.maxhealth = 1800;
            }
            else if ( var_0.sentrytype == "sentry_gun" )
            {
                var_0.maxhealth = 1600;
            }
        }

        // 基础血量设定完毕后 —— 新增
        if ( isdefined( level.sentryGunUpgradePurchased ) && level.sentryGunUpgradePurchased && isdefined( level.sentryGunHealthBonus ) )
        {
            var_0.maxhealth += level.sentryGunHealthBonus;
            var_0.health = var_0.maxhealth;
            iprintlnbold( "^2 [UPGRADE] Sentry bonus HP applied: +" + level.sentryGunHealthBonus + " -> " + var_0.maxhealth );
        }
    ...
    else if ( var_0 == "airsupport" )
    {
        switch ( var_1 )
        {
            ...
            case "specialty_stalker":
                var_2 = ::_id_3F2D;
                break;
            case "call_for_resupply":
                var_2 = mod_friendly_upgrade::function_call_for_resupply_allow;
                break;                
            case "upgrade_friendly_squad":
                var_2 = mod_friendly_upgrade::function_upgrade_friendly_squad_allow;
                break;
            case "upgrade_sentry_gun":
                var_2 = mod_friendly_upgrade::function_upgrade_sentry_gun_allow;
                break;
            default:
                break;
        }
    ...
    else if ( var_0 == "airsupport" )
    {
        switch ( var_1 )
        {
            ...
            case "specialty_stalker":
                ...
                break;
            case "call_for_resupply":
                var_2 = mod_friendly_upgrade::function_call_for_resupply;
                break; 
            case "upgrade_friendly_squad":
                var_2 = mod_friendly_upgrade::function_upgrade_friendly_squad;
                break;
            case "upgrade_sentry_gun":
                var_2 = mod_friendly_upgrade::function_upgrade_sentry_gun;
                break;
            default:
                break;
        }
...
_id_3F1A( var_0, var_1 )
{
    ...
    // 修改：防弹衣可多次购买，每次获得3F1A中的数值，但总量不得超过600
    if ( var_0 == "armor" )
        self._id_3F16["points"] = self._id_3F16["points"] + var_1;
    else
        self._id_3F16["points"] = var_1;

    self._id_3F19 = self._id_3F16["points"];
...
_id_3F22( var_0 )
{
    // 1. 装备槽检查：玩家 D-Pad→ 是否空闲
    if ( !_id_3EE9() )
        return 0;

    // 2. 友军直升机：一次只能存在一架
    if ( var_0 == "friendly_support_chopper" )
    {
        if ( isdefined( level.friendlyChopperExists ) && level.friendlyChopperExists )
            return 0;
    }

    // 3. 盟友小队总量限制：< 3 支
    if ( !isdefined( level.alliedSquadCount ) )
        level.alliedSquadCount = 0;

    return level.alliedSquadCount < 3;
}
...
_id_3F24( var_0 )
{
    ...
    if ( var_0 == "friendly_support_delta" )
        var_1 = "small_icon_survival_juggernaut";
 
    if ( var_0 == "friendly_support_riotshield" )
        var_1 = "small_icon_survival_gign";
 
    if ( var_0 == "friendly_support_chopper" )
        var_1 = "small_icon_survival_chopper";

    self setweaponhudiconoverride( "actionslot4", var_1 );
    notifyoncommand( "friendly_support_called", "+actionslot 4" );
 
    // 记录召唤新兵前的所有盟友,用于后续比对
    allies_before_spawn = getaiarray("allies");
 
    self waittill( "friendly_support_called" );
    // 根据类型决定召唤单位
    if ( var_0 == "friendly_support_chopper" )
    {
        level.friendlyChopperExists = 1;   // 立刻标记，防止短时间内重复购买
        thread spawn_friendly_attack_chopper();
    }
    else
        maps\_so_survival::spawn_allies( self.origin, var_0, self );
 
    // 记录召唤新兵后的所有盟友
    allies_after_spawn = getaiarray("allies");
    new_squad_members = []; // 用于存储新召唤的小队成员
 
    // 循环比对,找出新增的盟友单位,他们就是新召唤的小队
    foreach(ally in allies_after_spawn)
    {
        is_new = true;
        foreach(old_ally in allies_before_spawn)
        {
            if(ally == old_ally)
            {
                is_new = false;
        ...
        if(is_new)
        {
            new_squad_members[new_squad_members.size] = ally;
        }
    }
 
    if(new_squad_members.size > 0)
    {
        // 为这支新小队创建一个独立的"监控实体",专门负责追踪他们的生死
        monitor_entity = spawn("script_origin", (0,0,0));
        // 让这个"监控实体"开始运行小队监控函数
        monitor_entity thread monitor_squad_wipe(new_squad_members);
 
        if ( !isdefined( level.alliedSquadCount ) )
            level.alliedSquadCount = 0;
 
        // 全局小队计数器加1
        level.alliedSquadCount++;
		iprintlnbold( "^1 [LOG] Reinforcements Standing by.^3 Current Squads:" + level.alliedSquadCount);

        // 如果已购买升级，为这些新成员应用生命值加成
        if ( isdefined( level.friendlySquadHealthBonus ) && level.friendlySquadHealthBonus > 0 )
        {
            foreach ( member in new_squad_members )
            {
                member.maxhealth = member.maxhealth + level.friendlySquadHealthBonus;
...
monitor_squad_wipe(squad_members)
{
    // 在这个函数里, self 指的是那个为特定小队创建的、独一无二的"监控实体"
    self endon("death"); // 如果实体被意外删除,则终止线程
    level endon("special_op_terminated"); // 如果特殊行动结束,也终止
 
    // 为小队中的每一个AI成员,都启动一个独立的"死亡观察"线程
    foreach(member in squad_members)
    {
        member thread watch_for_death(self);
    }
    
    // "监控实体"在此等待,直到收到了所有成员的死亡通知
    // 小队有多少成员,就需要等待多少次 "squad_member_down" 信号
    for(i = 0; i < squad_members.size; i++)
    {
        self waittill("squad_member_down");
    }
 
    // 当循环结束,意味着小队已全员阵亡,此时减少全局计数器
    if(isdefined(level.alliedSquadCount) && level.alliedSquadCount > 0)
    
    {
        level.alliedSquadCount--;
        iprintlnbold("^1 [LOG] One of Ally Squad has fallen.^3 Current Squads:" + level.alliedSquadCount);
    }
    
    // 任务完成,删除这个"监控实体",清理游戏资源
    self delete();
}
 
watch_for_death(monitor_entity)
{
    // 在这个函数里, self 指的是运行此线程的AI单位本身
    self waittill("death"); // 等待这个AI死亡
 
    // 确认其对应的"监控实体"仍然存在
    if(isdefined(monitor_entity))
    {
        monitor_entity notify("squad_member_down");
    }
}
...
_id_3F2D( var_0 )
{
    ...
    // 增加限制：已有两个perk时，无法再购买
    if ( isdefined( self.custom_perks ) && self.custom_perks.size >= 2 )
        return 0;
...
spawn_friendly_attack_chopper()
{
    ...
    // 4.设置直升机的AI战斗逻辑
    chopper _id_0618::_id_3DCD( "friendly_support_delta" );
    chopper._id_3D5D = _id_061C::_id_3DF4( "chopper" );
    chopper [[ level._id_3B89 ]]();

    // ---- 将直升机和其炮塔彻底设置为友军阵营 ----
    chopper.team = "allies";
    chopper.pers["team"] = "allies"; // 持久化阵营设置
    chopper setthreatbiasgroup( "allies" ); // 设置威胁偏向组为友军
    chopper.script_team = "allies";
    chopper.targetname = "friendly_chopper"; // 设置目标名称
    chopper setvehicleteam( "allies" ); // 设置载具阵营
    chopper.ignorefriendlyfire = true; // 忽略友军火力

    // 启用炮塔、旋转、死亡动画等
    chopper._id_2942 = 1; 
    chopper thread maps\_vehicle::_id_2A3E(); // 启动炮塔AI更新线程

    // 检查并设置机枪炮塔的属性
    if ( isdefined( chopper.mgturret ) )
    {
        foreach ( turret in chopper.mgturret )
        {
            turret.team = "allies";
            turret.airank = "captain";
            turret.pers["team"] = "allies";
            turret setthreatbiasgroup( "allies" );
            turret setbottomarc( 120 ); // 设置垂直射击角度
            turret setturretteam( "allies" ); // 设置炮塔阵营
        }
    }

    // 巡逻与目标锁定
    chopper thread friendly_chopper_patrol();

    // 5. 启动寿命计时器：固定时间后返航并消失
    chopper thread friendly_chopper_lifetime( path.origin );
    iprintlnbold( "^3Friendly Chopper ^1incoming." );
...
friendly_chopper_lifetime( start_origin )
{
    ...
    // 离场并清除存在标记
    iprintlnbold( "^3Friendly Chopper,Short Fuel,^1Returning to Base." );
    level.friendlyChopperExists = 0;
    self delete();
}